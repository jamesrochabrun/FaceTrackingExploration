...
class GameScene: SKScene {
    ...
    func movePlayer (state:PlayerState) {
        ...
        if let player = playerNode {
            player.texture = SKTexture(imageNamed: state.rawValue) // name on assets
            
            var direction: CGFloat = 0
            switch state {
            case .up: direction = 116
            case .down: direction = -116
            case .neutral: direction = 0
            }
            // check for playe not go out of screen
